...
export const UGCLightbox: React.FC<Props> = ({ images, initialIndex, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] bg-black/95 flex items-center justify-center animate-pop" onClick={onClose}>
      <button 
        onClick={onClose} 
      ...
      <button onClick={prev} className="absolute left-2 md:left-8 p-3 text-white/50 hover:text-white transition-colors z-20">
      ...
      <button onClick={next} className="absolute right-2 md:right-8 p-3 text-white/50 hover:text-white transition-colors z-20">
         <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
